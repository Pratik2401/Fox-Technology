...
const issueController = {
    ...
    async getAll(req, res) {
        ...
        SELECT bi.*, b.name as book_name, b.payment_status, m.name as member_name 
        FROM book_issues bi 
    ...
    async issueBook(req, res) {
        try {
            ...
            const [result] = await db.execute(
                'INSERT INTO book_issues (book_id, member_id, issue_date, return_date, return_status, lost_status, lost_reason) VALUES (?, ?, NOW(), ?, 0, 0, ?)', [book_id, member_id, return_date, lost_reason || '']
            );
    ...
    async update(req, res) {
        try {
            const { issue_id } = req.params;
            const { book_id, member_id, return_date, actual_return_date, lost_status, lost_reason } = req.body;

            // Get current issue data to check for lost status changes
            const [currentIssue] = await db.execute('SELECT book_id, member_id, lost_status FROM book_issues WHERE issue_id = ?', [issue_id]);
            if (currentIssue.length === 0) {
                return res.status(404).json({ error: 'Issue record not found' });
            }

            const currentLostStatus = currentIssue[0].lost_status;
            const issueBookId = currentIssue[0].book_id;
            const issueMemberId = currentIssue[0].member_id;

            if (book_id && member_id) {
                const [updateResult] = await db.execute(
                    'UPDATE book_issues SET book_id = ?, member_id = ?, return_date = ?, actual_return_date = ?, lost_status = ?, lost_reason = ? WHERE issue_id = ?', [book_id, member_id, return_date, actual_return_date, lost_status, lost_status ? lost_reason || '' : '', issue_id]
                );
                ...
                const [updateResult] = await db.execute(
                    'UPDATE book_issues SET return_date = ?, actual_return_date = ?, lost_status = ?, lost_reason = ? WHERE issue_id = ?', [return_date, actual_return_date, lost_status, lost_status ? lost_reason || '' : '', issue_id]
                );
            ...
            // Handle lost status changes
            if (currentLostStatus !== lost_status) {
                if (lost_status) {
                    // Changed to lost - add fee and set payment pending
                    const [bookData] = await db.execute('SELECT price FROM books WHERE book_id = ?', [issueBookId]);
                    const bookCost = bookData[0]?.price || 0;
                    await db.execute('UPDATE members SET lost_book_fee_amount = lost_book_fee_amount + ? WHERE member_id = ?', [bookCost, issueMemberId]);
                    await db.execute('UPDATE books SET lost_status = TRUE, payment_status = "Pending" WHERE book_id = ?', [issueBookId]);
                } else {
                    // Changed from lost to not lost - remove fee and reset payment status
                    const [bookData] = await db.execute('SELECT price FROM books WHERE book_id = ?', [issueBookId]);
                    const bookCost = bookData[0]?.price || 0;
                    await db.execute('UPDATE members SET lost_book_fee_amount = lost_book_fee_amount - ? WHERE member_id = ?', [bookCost, issueMemberId]);
                    await db.execute('UPDATE books SET lost_status = FALSE, payment_status = "Not Lost" WHERE book_id = ?', [issueBookId]);
                }
            }
    ...
    async toggleLostStatus(req, res) {
      ...
      `, [issue_id]);
            ...
            await db.execute(
                'UPDATE book_issues SET lost_status = ?, lost_reason = ? WHERE issue_id = ?', [lost_status, lost_reason || '', issue_id]
            );

            if (lost_status) {
                const bookCost = issueRecord[0].price || 0;
                await db.execute(
                    'UPDATE members SET lost_book_fee_amount = lost_book_fee_amount + ? WHERE member_id = ?', [bookCost, issueRecord[0].member_id]
                );

                const [bookData] = await db.execute('SELECT book_id FROM book_issues WHERE issue_id = ?', [issue_id]);

                // Set book as lost and payment status as Pending
                await db.execute(
                    'UPDATE books SET lost_status = TRUE, payment_status = "Pending" WHERE book_id = ?', [bookData[0].book_id]
                );
            } else {
                // If removing lost status, reset payment status
                const [bookData] = await db.execute('SELECT book_id FROM book_issues WHERE issue_id = ?', [issue_id]);

                await db.execute(
                    'UPDATE books SET lost_status = FALSE, payment_status = "Not Lost" WHERE book_id = ?', [bookData[0].book_id]
                );
            }
    ...
    async markPaymentDone(req, res) {
        try {
            ...
            await db.execute(
                'UPDATE books SET payment_status = "Paid" WHERE book_id = ?', [bookData[0].book_id]
            );
